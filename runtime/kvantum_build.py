...
Flags:
  --hover=soft    Soft hover highlight (default): low-opacity tint
  --hover=border  No hover background; thin border around hovered item
"""
import json
import re
import sys
...
MAP = {
    ...
    #"#00bcd4": C2, "#009DFF": C12, "#9E4FFF": C13, "#eefcff": FG,
...
def replace_all(text: str) -> str:
    for hexv in sorted(MAP, key=len, reverse=True):
        text = re.sub(re.escape(hexv), MAP[hexv], text, flags=re.IGNORECASE)
    for lit, val in LITERAL.items():
        text = re.sub(rf'(=\s*){lit}\b', rf'\1{val}', text)
    return text


def apply_post_override(text: str) -> str:
    for key, val in POST_OVERRIDE.items():
        text = re.sub(
            rf'^{re.escape(key)}\s*=.*$',
            f'{key}={val}',
            text,
...
def patch_itemview_kvconfig(text: str) -> str:
...
def patch_itemview_svg(text: str) -> str:
...
def main() -> None:
    ...
    src = replace_all(src)
    src = apply_post_override(src)
    src = patch_itemview_kvconfig(src)
    if HOVER == "border":
        src = src.replace(
            "[ItemView]\n",
            f"[ItemView]\nframe.color={C2}\nframe.expansion=0\n",
    ...
    print(f"Written: {OUT_DIR / 'pywal.kvconfig'}  [hover={HOVER}]")

    # svg
    src = (ADAPTA / "KvAdaptaDark.svg").read_text()
    src = replace_all(src)
    src = patch_itemview_svg(src)
